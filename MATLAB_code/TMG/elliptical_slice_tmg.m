function [xx, cur_log_like, number_fn_evaluations, nu] = elliptical_slice_tmg(xx, prior, log_like_fn, cur_log_like, F, g, EP_mean,dimension)

% This is an attempt to adapt the code for the tmg case.
% The angle range is computed inside evry time
...
% Compute the angle range from Ari's code
% Input nu to Wall_hitting to compute the hitting times
...
% Instead of randomly choosing an initial point, want to choose from within
% the slice.
    ...
    % Randomization here is necessary to maintain detail balance
    % See ESS paper for details
...
if run_ess == 1
    ...
    % The entire ellipse was within the box -- do regular EPESS
    run_ess
    while true
...
%              phi = simulate(angle_slice_2, phi_min, phi_max);
