function [output, cur_log_like, number_fn_evaluations, nu] = ess_recycle(xx, prior, log_like_fn, cur_log_like, N, dimension, angle_range, varargin)

%% This idea is a general modification of (elliptical) slice sampling with
%  the recycling idea of David Dunson

%% As a test, for N=1, this gives the same performance as EPESS

% Let N be the number of points we want to get per shrinking step. We wil
% choose uniformly among the N points to continue the Markov Chain sampler.
...
output = zeros(N, dimension);


% As we have N >1, somtimes we have to shrink a lot.
% For large N, the phi_min and phi_max become smaller than machine 
% tolerance. 

% Using tol, we are truncating the sice shrinkage to avoid large lops and
% errors
...
for counter = 1:N 
    ...
    % Using current value of phi_min and phi_max, get a new proposal and
    % continue the markv chain
    if counter > 1
        phi = rand*(phi_max - phi_min) + phi_min;
    end 
