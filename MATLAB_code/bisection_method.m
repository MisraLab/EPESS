function [ bound, number_fn_evals, probe_points, probe_log_thresholds, initial_boundary ] =  ...
    bisection_method(logLikelihoodDirection, previous_initial_boundary, tol)

    % Initialize
    number_fn_evals = 0;
    initial_boundary = 0; % This is the boundary of the outermost slice. If there are previous points then this is not included in any calculations.
    
    if isempty(previous_probe_points)
        
        % Initialize points
        number_probes = 2;
        probe_points(1) = 0;
        probe_points(2) = previous_initial_boundary;
        ...
        % Intialize
        probe_points = previous_probe_points;
        probe_log_thresholds = previous_probe_log_thresholds;
        ...
        % Remove points that are larger than outer. These will never be
        % used again, so no point in keeping them. If you don't do this
        % then it can grow very long if J = length(u) is large.
    ...
    if bound~= mid_point % This will happen if break is activated above.
        bound = inner_point;
    end
